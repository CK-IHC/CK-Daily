 ...
 * grid-auto-rows:1fr บังคับทุกแถวในตารางสูงเท่ากัน + .receipt เป็น flex column สูงเต็ม cell (height:100%)
 * แล้วดันยอดสุทธิ (.total) ไปชิดล่างด้วย margin-top:auto — ทำให้ทุกใบเสร็จมีกรอบสูงเท่ากันสมมาตรกันเสมอ
 * ไม่ว่าใบไหนจะมีรายการสินค้ามากน้อยต่างกัน (เดิมใบที่มีรายการน้อยจะดูสั้นกว่าใบข้างเคียง)
 */
var RECEIPT_GRID_STYLE_ =
  '@page{size:A4 landscape;margin:8mm}' +
  '.a4-sheet{display:grid;grid-auto-rows:1fr;gap:6mm;page-break-after:always;align-content:stretch}' +
  '.a4-sheet .receipt{max-width:none;width:100%;border:1px dashed #bbb;border-radius:6px;padding:12px;box-sizing:border-box;height:100%;display:flex;flex-direction:column}' +
...
function printOrdersReport(items, total) {
  ...
  var rows = items.map(function (o) {
    return '<tr><td>#' + UI.escapeHtml(o.order_no) + '</td><td>' + UI.escapeHtml(o.customer_name) + '</td><td>' + UI.escapeHtml(o.phone) + '</td><td>' + UI.escapeHtml(o.department || '-') + '</td>' +
      '<td style="text-align:right">' + UI.money(o.grand_total) + '</td><td>' + UI.statusLabel(o.status) + '</td></tr>';
  }).join('');
  ...
  var html = '<!DOCTYPE html><html><head><title>รายงานออเดอร์</title><meta charset="utf-8">' +
    '<style>body{font-family:Arial,sans-serif;padding:24px 24px 46px;color:#000}' +
    ...
    'th{background:#f3f4f6}tfoot td{font-weight:800;border-top:2px solid #333}</style></head><body>' +
    ...
    '<table><thead><tr><th>เลขที่</th><th>ลูกค้า</th><th>เบอร์</th><th>แผนก</th><th style="text-align:right">ยอด</th><th>สถานะ</th></tr></thead><tbody>' + rows + '</tbody>' +
    '<tfoot><tr><td colspan="4">รวม</td><td style="text-align:right">' + UI.money(sum) + '</td><td></td></tr></tfoot></table>' +
    '<div class="print-doc-footer">' + printedByHtml_() + '</div>' +
...
Views.orders = function (container) {
  ...
  function renderDetail(data) {
    ...
    el.innerHTML = '<div class="table-wrap"><table><thead><tr><th>เลขที่</th><th>ลูกค้า</th><th>เบอร์</th><th>ยอด</th><th>ชำระเงิน</th><th>สถานะ</th><th>เวลา</th><th>รายการสั่งซื้อ</th><th class="no-print">จัดการ</th></tr></thead><tbody>' +
