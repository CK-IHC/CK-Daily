...
function printOrderHistoryReport_(items, tabLabel) {
  ...
  var rows = items.map(function (o) {
    return '<tr><td>#' + UI.escapeHtml(o.order_no) + '</td><td>' + UI.fmtTime(o.placed_at) + '</td>' +
      '<td>' + (STATUS_LABEL_TH_[o.status] || o.status) + '</td><td style="text-align:right">' + UI.money(o.grand_total) + '</td></tr>';
  }).join('');
  ...
  var html = '<!DOCTYPE html><html><head><title>ประวัติการสั่งซื้อ</title><meta charset="utf-8">' +
    ...
    '<table><thead><tr><th>เลขที่</th><th>เวลา</th><th>สถานะ</th><th style="text-align:right">ยอด</th></tr></thead><tbody>' + rows + '</tbody>' +
    '<tfoot><tr><td colspan="3">รวม</td><td style="text-align:right">' + UI.money(sum) + '</td></tr></tfoot></table>' +
